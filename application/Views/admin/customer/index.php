...
<?php endif; ?>  
<div class="heading">Danh sách khách hàng | <a href="<?php echo url("admin/customer/add"); ?>">Thêm khách hàng</a></div>
<div class="row">
    <div class="row col-12 pr-0">
        <div class="col-7">
            <form method="GET" class="mb-3">
                <input type="text" name="keyword" value="<?php echo get_query("keyword") ?? "" ?>" class="form-control" placeholder="Tìm khách hàng theo tên hoặc số điện thoại">
            </form>  
        </div>
        <div class="col-5 text-right pr-0">
            <a href="<?php echo url("admin/customer/export/excel" . "?" . get_query_string(current_url())); ?>" class="btn btn-success btn-icon-split">
    ...
    <div class="col-12">
        <table id="cat-table" class="table table-borderless position-relative">
            <thead>
                <tr>
                    <th style="width:20%" scope="col">Mã khách hàng</th>
                    <th style="width:30%" scope="col">Họ tên</th>
                    <th style="width:30%" scope="col">Số điện thoại</th>
                    ...
                    <tr>
                        <td>
                            ...
                            <div class="row-actions">
                                <span class="delete"><a href="javascript:void(0)" onclick="deleteCustomer(<?php echo $item->id ?>)">Xóa</a><form id="delete-customer-<?php echo $item->id ?>" style="display:none" method="POST" action="<?php echo url("admin/customer/delete/{$item->id}") ?>"><input type="hidden" name="csrf_token" value="<?php echo csrf_token() ?>"></form></span>  | 
                                <span class="edit"><a href="<?php echo url("admin/customer/edit/{$item->id}") ?>">Cập nhật</a></span>
                            </div>
                        </td>
                        <td>
                           <?php echo $item->name; ?>
                        ...
                        <td>
                           <a href="<?php echo url("admin/customer/{$item->id}/purchase/history"); ?>">Lịch sử mua hàng</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
