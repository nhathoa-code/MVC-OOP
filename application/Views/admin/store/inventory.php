...
<div class="heading">Kho: <?php echo $store->name ?> | <span><a href="<?php echo url("admin/store/{$store->id}/inventory/add") ?>">Nhập kho</a></span></div>
<div class="row">
    <div class="row col-12 pr-0">
        ...
        <div class="col-5 text-right pr-0">
            <a href="<?php echo url("admin/store/{$store->id}/inventory/export/excel" . "?" . get_query_string(current_url())); ?>" class="btn btn-success btn-icon-split">
                    ...
                    <tr id="<?php echo $p->id; ?>">
                        ...
                        <td class="product-variation-stock">
                            ...
                            <?php elseif($p->sizes): ?>
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <div><?php echo $p->stock[0]->stock; ?></div>
                            <?php endif; ?>
                        </td>
                        ...
                        <td class="product-variation-price">
                            ...
                            <?php elseif($p->sizes): ?>
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <div><?php echo number_format($p->stock[0]->price,0,"",".") . "₫"; ?></div>
                            <?php endif; ?>
                        </td>
                        <td class="product-actions">
                            <a href="<?php echo url("admin/store/{$store->id}/inventory/product/{$p->id}/edit") ?>">Cập nhật</a>
                            <a href="<?php echo url("product/detail/{$p->id}") ?>">Xem sản phẩm</a>
                            <a style="color:brown" href="javascript:void(0)" onclick="deleteStoreInventory(<?php echo $store->id ?>,'<?php echo $p->id ?>')">Xóa khỏi kho</a>
                            <form id="delete-store-inventory-<?php echo $store->id ?>-product-<?php echo $p->id ?>" style="display:none" method="POST" action="<?php echo url("admin/store/{$store->id}/inventory/product/{$p->id}/delete") ?>"><input type="hidden" name="csrf_token" value="<?php echo csrf_token() ?>"></form>
                        </td>
                    </tr>
                <?php endforeach; ?>    
            </tbody>
